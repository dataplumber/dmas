...
import gov.nasa.podaac.common.api.jaxb.mmr.MMRFR;
import gov.nasa.podaac.common.api.jaxb.mmr.MMRFR.Personnel;
...
import gov.nasa.podaac.common.api.transformer.TransformerAux;
...
public class MMRTransformer extends Binder<MMRFR> implements Transformer,
   ...
   private static final String JAXB_CONTEXT =
         "gov.nasa.podaac.common.api.jaxb.mmr";
   ...
   private MessageLevel _contributorMessageLevel = MessageLevel.VERBOSE;
    ...
    */
   protected synchronized CompressionAlgorithm _getCompression(String filename) {

      Hashtable<String, CompressionAlgorithm> table =
            new Hashtable<String, CompressionAlgorithm>();
      table.put("GZIP", CompressionAlgorithm.GZIP);
      table.put("BZIP2", CompressionAlgorithm.BZIP2);
      table.put("ZIP", CompressionAlgorithm.ZIP);
   ...
   protected void _processGranuleFile(GranuleFile gfile, URIPath dataFile,
      ...
      try {
         ...
         info.setLink(new URI(link));
      } catch (URISyntaxException e) {
         if (MMRTransformer._logger.isDebugEnabled())
            e.printStackTrace();
         throw new MMRException("Unable to create URI from file name: " + link
               + ".");
      }
      info.setSize(size);
      CompressionAlgorithm comp = this._getCompression(dataFile.getURI());
      if (comp != null) {
         info.setCompressionAlgorithm(comp);
      }
      ...
      if (checksumFile != null) {
         try {
            ...
            ChecksumAlgorithm alg =
                  ChecksumAlgorithm.valueOf(checksumFile.getURI().substring(
                        checksumFile.getURI().lastIndexOf('.') + 1,
                        checksumFile.getURI().length()).toUpperCase());
            if (checksum != null && alg != null) {
               info.setChecksum(checksum.substring(0, checksum.indexOf(' ')));
               info.setChecksumAlgorithm(alg);
            }
         } catch (IOException e) {
            if (MMRTransformer._logger.isDebugEnabled())
               e.printStackTrace();
   ...
   protected synchronized ServiceProfile _transform(MMRFR jaxbObj)
      ...
      try {
         ...
         ProductProfile productProfile = profile.createProductProfile();
         profile.setProductProfile(productProfile);

         Agent origin = profile.createAgent();
         origin.setAgent(this.getClass().getName());

         Agent target = profile.createAgent();
         target.setAgent("ALL");
         ...
         for (Personnel person : personnel) {
            Notification notify = productProfile.createNotification();
            ...
            notify.setMessageLevel(MessageLevel.SILENT);
            notify.setPhone(person.getPhone());

            productProfile.addNotification(notify);
         }

         IngestProfile ingestProfile = productProfile.createIngestProfile();
         productProfile.setIngestProfile(ingestProfile);

         SubmissionHeader header = ingestProfile.createHeader();
         ingestProfile.setHeader(header);

         CompleteContent content = ingestProfile.createCompleteContent();
         ingestProfile.setCompleteContent(content);

         header.setProject(jaxbObj.getEntryID());
         header.setBatch(jaxbObj.getEntryID() + "_"
               + System.currentTimeMillis());
         header.setContributorEmail(this._contributorEmailAddress);
         header.setContributorMessageLevel(this._contributorMessageLevel);
         ...
         granule.setDatasetName(jaxbObj.getEntryID().trim());
         granule.setProductType("GHRSST-"+jaxbObj.getEntryID().trim());
         granule.setName(gname);
         granule.setVersion(jaxbObj.getFileVersion().trim());
         granule.setCreateTime(this._getDate(jaxbObj.getFileReleaseDate()));

         granule.setTemporalCoverageStartTime(this._getDate(jaxbObj
               .getTemporalCoverage().getStartDate()));
         granule.setTemporalCoverageStopTime(this._getDate(jaxbObj
               .getTemporalCoverage().getStopDate()));

         if (granule.getTemporalCoverageStartTime().getTime() < 0 ||
               granule.getTemporalCoverageStartTime().getTime() > granule.getTemporalCoverageStopTime().getTime()) {
         ...
         BoundingRectangle boundingRectangle =
               granule.createBoundingRectangle();
         granule.setBoundingRectangle(boundingRectangle);
         ...
         // process RAW file
         GranuleFile gfile = granule.createFile();
         granule.addFile(gfile);
         ...
         gfile.setName(jaxbObj.getFileName());
         this._processGranuleFile(gfile, rawFile, checksumFile);
         gfile.setType(FileClass.DATA);

         // process RAW checksum file
         gfile = granule.createFile();
         granule.addFile(gfile);
         if (checksumFile.getFilename() == null) {
            throw new MMRException("URI contains no file name.");
         }
         gfile.setName(checksumFile.getFilename());
         this._processGranuleFile(gfile, checksumFile, null);
         gfile.setType(FileClass.CHECKSUM);
         ...
         gfile.setName(mmrFile.getFilename());
         this._processGranuleFile(gfile, mmrFile, mmrChecksumFile);
         gfile.setType(FileClass.METADATA);
         ...
         gfile.setName(mmrChecksumFile.getFilename());
         this._processGranuleFile(gfile, mmrChecksumFile, null);
         gfile.setType(FileClass.CHECKSUM);
