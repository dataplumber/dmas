...
public class ServiceProfileFactoryTest {
   ...
   /**
    * Test to create ServiceProfile from an input SPM file.
    */
   /*
   @Test
   public void createServiceProfileFromPackageWithFile() {
      // creating ServiceProfile object with File object
      ServiceProfileFactory spf = ServiceProfileFactory.getInstance();
      try {
         ServiceProfile serviceProfile =
               spf.createServiceProfile(new SPMTransformer(), new File(
                     this._rootPath + SPM_FILE_NAME));
         assertNotNull(serviceProfile);
      } catch (Exception exception) {
         fail("Failed to create ServiceProfile object.");
      }
   ...
   */
   ...
   /**
    * Test to create ServiceProfile from an input SPM text string.
    */
   /*
   @Test
   public void createServiceProfileFromPackageWithString() {
      // read in package
      StringBuilder spmContent = new StringBuilder();

      try {
         FileReader fileReader =
               new FileReader(new File(this._rootPath + SPM_FILE_NAME));
         BufferedReader bufferedReader = new BufferedReader(fileReader);

         String line;
         while ((line = bufferedReader.readLine()) != null) {
            spmContent.append(line + "\n");
         }

         bufferedReader.close();
         fileReader.close();
      } catch (Exception exception) {
         fail("Failed to read in SPM.");
      }
      ...
      try {
         ServiceProfile serviceProfile =
               spf.createServiceProfile(new SPMTransformer(), spmContent
                     .toString());
         assertNotNull(serviceProfile);
      } catch (Exception exception) {
         fail("Failed to create ServiceProfile object.");
