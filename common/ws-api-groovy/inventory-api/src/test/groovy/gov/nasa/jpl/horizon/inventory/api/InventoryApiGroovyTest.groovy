...
public class InventoryApiGroovyTest extends GroovyTestCase{
	...
	def HOST = "http://lanina:9191";
	
	public void testFetchDatasets() {
		println "Running Fetch Datasets Test"
		InventoryApi api = new InventoryApi(HOST);
		List<Map<String,String>> providers = api.getDatasets();
	...
	public void testUpdateGranuleStatus(){
		...
		InventoryApi api = new InventoryApi(HOST);
		def ds = api.updateGranuleStatus(3729599,"OFFLINE","axt","axt388");
		assert ds == true
		
		api.setAuthInfo("axt","axt388")
		ds = api.updateGranuleStatus(3729599,"ONLINE");
