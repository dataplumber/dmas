...
class ResponseDojoJson extends Response {
   ...
   public String toString() {
      ...
      if(parameters[ResponseDojoJson.ParameterLabel]) {
         response += '"identifier": '
         response += '"'+StringEscapeUtils.escapeJavaScript(parameters[ResponseDojoJson.ParameterLabel].value)+'",'
         response += '"label": '
         response += '"'+StringEscapeUtils.escapeJavaScript(parameters[ResponseDojoJson.ParameterLabel].value)+'",'
      }
      ...
      parameters.eachWithIndex {key, parameter, index ->
         if(key != ResponseDojoJson.ParameterLabel) {
            String entry = ""
