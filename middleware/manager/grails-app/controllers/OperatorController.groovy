...
class OperatorController {
   ...
   def authenticate = {
      def response = datasetService.authenticate(params.userName, params.password)
      render response as JSON
   }
   ...
   private boolean isAuthenticated(def userName, def password) {
      def response = datasetService.authenticate(userName, password)
      return DatasetService.RESPONSE_OK.equals(response.Status)
   }
