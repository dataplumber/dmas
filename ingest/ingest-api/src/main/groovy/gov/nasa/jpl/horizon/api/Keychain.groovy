...
class Keychain extends JAXBBinder {
   ...
   "gov.nasa.jpl.horizon.api.jaxb.keychain"
   ...
   def Keychain() {
      ...
      try {
         jaxbObj = new gov.nasa.jpl.horizon.api.jaxb.keychain.Keychain()
   ...
   synchronized void toFile(String filename) throws IOException {
      ...
      fedMap.each {key, value ->
         jaxbObj.key << new gov.nasa.jpl.horizon.api.jaxb.keychain.Keychain.Key(federation: key, username: value.username, password: value.password)
      }
