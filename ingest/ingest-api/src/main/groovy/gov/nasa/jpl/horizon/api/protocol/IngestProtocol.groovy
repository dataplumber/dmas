package gov.nasa.jpl.horizon.api.protocol

import gov.nasa.jpl.horizon.api.*
import gov.nasa.jpl.horizon.api.jaxb.protocol.*
import javax.xml.bind.JAXBException
import org.apache.commons.logging.Log
import org.apache.commons.logging.LogFactory

/**
 * Implementation of the Horizon wire protocol that supports marshalling
...
class IngestProtocol extends JAXBBinder {
  ...
  static final String PROTOCOL_SCHEMA = "podaac_ingest_protocol.xsd"
  static final String JAXB_CONTEXT =
  "gov.nasa.jpl.horizon.api.jaxb.protocol"
  ...
  protected List convertToFileList(List files) {
    List result = []

    files.each {
      CommonFileType newfile = new CommonFileType()
      newfile.name = it.name
      newfile.source = it.source
      newfile.sourceUsername = it.sourceUsername
      newfile.sourcePassword = it.sourcePassword
      newfile.maxConnections = it.maxConnections
      newfile.destination = it.destination
      newfile.size = BigInteger.valueOf(it.size)
      newfile.checksum = it.checksum

      CommonTimeStampType timestamp = new CommonTimeStampType()

      if (it.startTime) {
        newfile.processTime = timestamp
        timestamp.start = BigInteger.valueOf(it.startTime.time)
      }

      if (it.stopTime) {
        newfile.processTime = timestamp
        timestamp.stop = BigInteger.valueOf(it.stopTime.time)
      }

      if (it.description) {
        newfile.description = it.description
      }
  ...
  protected List convertToProductFileList(List files) {
    List result = []
    ...
    files.each {
      ProductFile file = new ProductFile(
              name: it.name,
              source: it.source,
              sourceUsername: it.sourceUsername,
              sourcePassword: it.sourcePassword,
              maxConnections: it.maxConnections,
              destination: it.destination,
              size: it.size.longValue(),
              checksum: it.checksum,
              description: it.description
      )

      if (it.processTime) {
        if (it.processTime.start)
          file.startTime = new Date(it.processTime.start.toLong())

        if (it.processTime.stop)
          file.stopTime = new Date(it.processTime.stop.toLong())
      }
  ...
  void setAddFiles(List files) throws ProtocolException {
    if (!add.files) {
      add.files = new Horizon.Add.Files()
    }

    add.files.file = convertToFileList(files)
  }

  List getAddFiles() throws ProtocolException {
    if (add.files) {
   ...
   */
  void setTimeRange(List timeRange) throws ProtocolException {
    if (!listType.timeRange) {
  ...
  synchronized List getProducts() throws ProtocolException {
    List result = []
