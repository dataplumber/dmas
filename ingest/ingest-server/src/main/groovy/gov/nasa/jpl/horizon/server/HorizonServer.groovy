...
import gov.nasa.podaac.common.util.*
import gov.nasa.jpl.horizon.util.*
...
import gov.nasa.podaac.common.api.util.ChecksumUtility.DigestAlgorithm
...
class HorizonServer {
  ...
  static Log log = LogFactory.getLog(HorizonServer.class)

  private static final INSTANCE = new HorizonServer()
  ...
  void activate(args) throws IOException {
    ...
    Thread.currentThread().name = "HORIZON"
    ...
    ServerMonitor.instance.userListener.add = {IngestProtocol request ->
      ...
      for (ProductFile productFile in productFiles) {
        log.trace("retrieve file ${productFile.name}")
        URIPath path = URIPath.createURIPath(productFile.source)
        def lock
        synchronized (locks) {
          lock = locks.get(path.hostPath)
          if (!lock) {
            lock = new java.util.concurrent.Semaphore(productFile.maxConnections, true)
        ...
        log.trace("Trying to fetch source file on local file system: "+"file://"+path.getPath())
        ...
        if(fileProduct.exists()) {
          if(productFile.checksum) {
            ...
            if(productFile.checksum.equalsIgnoreCase(checksum)) {
              foundLocally = true
              log.trace("File found on local file system and checksum matched!")
            }
          }
        }
        if(!foundLocally) {
        ...
        */
        ...
        if (!fileProduct.exists()) {
          ...
          log.trace("${productFile.name} expected size=${productFile.size}, actual size=${fileProduct.size}")
          if (productFile.size != fileProduct.getSize()) {
            log.trace("file size mismatched")
            errno = Errno.INGEST_ERR
            ...
            log.trace("Retrieve ${productFile.name} to ${destination.path}")
            ...
            if((fileToCopy.exists()) &&
              ...
              log.trace("Done retrieve file ${filename}... compute checksum")
  
              savedFiles << filename
  
              String checksum = ChecksumUtility.getDigest(DigestAlgorithm.MD5, new File(filename))
  
              log.trace("computed checksum ${checksum}")
      ...
      String content = ''
      */
      log.trace("UPDATE: ${request.toRequest()}")
      try {
  ...
  void startZooKeeperConnection(boolean isIngest) {
     ...
     try {
        ...
        //check if path to storage exists /engines/[storageName]
        if (!zk.nodeExists("${ZkAccess.ENGINE_REGISTRATION_ROOT}/$storageName")) {
           log.error("Unable to register engine $serverName with zookeeper for storage $storageName because path ${ZkAccess.ENGINE_REGISTRATION_ROOT}/$storageName does not exist")
           return
        }
        
        String path = zk.register(storageName, serverName)
        if (path == null) {
        ...
        public void run() {
           ...
           while (true) {
              if (i < maxWaitTime)
                 i += 1000
              try {
                 log.trace("Poll ZK at storage " + storageName)
                 //check zookeeper see if engine is still registered
                 switch (zk.checkRegistration(storageName, serverName)) {
                    case RegistrationStatus.READY:
                       ...
                       if (input != null) {
                          ...
                          if (manifest.operation == Opcode.ENGINE_INGEST) {
                             ...
                             log.trace("Finished calling ServerMonitor.instance.userListener.add")
                          } else if (manifest.operation == Opcode.ENGINE_DELETE) {
                             def name = "${manifest.jobId}:${manifest.federation}:${manifest.productType}:${manifest.product}"
                             log.trace("Working on " + name)
                             ServerMonitor.instance.userListener.delete(manifest)
                             log.trace("Finished calling ServerMonitor.instance.userListener.delete")
                          } else if (manifest.operation == Opcode.ENGINE_MOVE) {
                             def name = "${manifest.jobId}:${manifest.federation}:${manifest.productType}:${manifest.product}"
                             log.trace("Working on " + name)
                             ServerMonitor.instance.userListener.move(manifest)
                             log.trace("Finished calling ServerMonitor.instance.userListener.move")
                          }
                       } else {
  ...
  public static void main(args) throws Exception {
    HorizonServer.instance.activate(args)
  }
