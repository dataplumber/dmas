...
class FileGrabberTest extends GroovyTestCase {
   ...
   def interfaces = [gov.nasa.podaac.common.api.file.FileProductHandler]
  ...
  /*
   void testGrabFiles() {
      ...
      [
         ...
         manager.registerCrawler(createCrawler(it, "podaacdev", null, 1))
      }
   ...
   */
   ...
   void testCrawl () {
      ...
      10.times {
         ...
         crawler.setAuthentication('podaacdev', 'perfect$storm')
         filter = new gov.nasa.podaac.common.api.file.FileFilter((String[]) ['\\S*.md5$', '\\S*.xml$', '\\S*.bz2$'])
   ...
   Crawler createCrawler(String url, String username, String password, int maxCon) {
      ...
      filter = new gov.nasa.podaac.common.api.file.FileFilter((String[]) [
            path.filename
      ])
      crawler.registerProductSelector(filter)
