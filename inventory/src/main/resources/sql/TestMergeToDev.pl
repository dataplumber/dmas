...
print "Oracle SID:       DAACTEST\n";
print "Username:         podaac_test\n";
print "--------------------------------------------------------\n\n";
...
sub getDBH{
	  my $dbh =  DBI->connect("DBI:Oracle:host=seadb.jpl.nasa.gov;sid=DAACTEST",'podaac_test','podaac$test')
...
sub printDatasetInfo {
	...
	print "Extracting datataset: $row[2]";  	
	  ...
	  print MYFILE "INSERT INTO DATASET (DATASET_ID, PROVIDER_ID, SHORT_NAME, LONG_NAME, ORIGINAL_PROVIDER, PROVIDER_DATASET_NAME, PROCESSING_LEVEL, LATITUDE_RESOLUTION,LONGITUDE_RESOLUTION,HORIZONTAL_RESOLUTION_RANGE, ALTITUDE_RESOLUTION, DEPTH_RESOLUTION, TEMPORAL_RESOLUTION, TEMPORAL_RESOLUTION_RANGE, ELLIPSOID_TYPE,PROJECTION_TYPE, PROJECTION_DETAIL, REFERENCE, DESCRIPTION,ACROSS_TRACK_RESOLUTION, ALONG_TRACK_RESOLUTION,ASCENDING_NODE_TIME, REMOTE_DATASET )values ($row[0],$row[1],'$row[2]','$row[3]','$row[4]','$row[5]','$row[6]',$row[9],$row[10],'$row[11]','$row[12]','$row[13]','$row[14]','$row[15]','$row[16]','$row[17]','$row[18]','$row[19]','$row[20]','$row[21]','$row[22]','$row[23]', '$row[24]');\n"; 
        }
	...
	$SEL = "SELECT DATASET_ID, ELEMENT_ID, OBLIGATION_FLAG, DE_ID, SCOPE FROM DATASET_ELEMENT where DATASET_ID=$d";
		...
		$arraySize = @row;
                ...
                print MYFILE "INSERT INTO DATASET_ELEMENT (DATASET_ID, ELEMENT_ID, OBLIGATION_FLAG, DE_ID, SCOPE)values ($row[0],$row[1],'$row[2]',DATASET_ELEMENT_ID_SEQ.NEXTVAL,'$row[4]');\n";
        }
