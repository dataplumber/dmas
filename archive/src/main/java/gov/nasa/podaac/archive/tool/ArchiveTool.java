 ...
 *
 * @author clwong
 * $Id: ArchiveTool.java 13212 2014-04-25 18:27:09Z gangl $
 */
public class ArchiveTool {
	...
	private static OptionGroup createDatasetOptions() {
		OptionGroup optionGroup = new OptionGroup( );		
		optionGroup.addOption(OptionBuilder.withArgName( "datasetId" )
				.withType(new Integer(0))
                .hasArg()
                .withLongOpt("datasetId")
                .withDescription("Dataset Id" )
                .create("d"));
		/* Take these options out because it can be hugh impact.
		optionGroup.addOption( OptionBuilder.withArgName( "id1,id2,id3,..." )
                .hasArgs()
                .withValueSeparator(',')
                .withDescription("List of dataset id" )
                .create("dl") );	
		optionGroup.addOption( OptionBuilder.withArgName( "beginId:endId" )
                .hasArgs(2)
                .withValueSeparator(':')
                .withDescription("Range of dataset id" )
                .create("dr") );
                */
		return optionGroup;
	}
	...
	private static Options createDeleteCmdOptions() {
		...
		options.addOption(OptionBuilder.hasArg(false)
                ...
                .withDescription("Delete Archived data only (default)" )
                .create());
		options.addOption(OptionBuilder.hasArg(false)
				.withLongOpt("delete-metadata")
                .withDescription("Delete Inventory metadata" )
                .create());
		options.addOption(OptionBuilder.hasArg(false)
	...
	private static OptionGroup createEntityOptions() {
		OptionGroup optionGroup = new OptionGroup();
		Iterator it = createGranuleOptions().getOptions().iterator();
		while (it.hasNext()) optionGroup.addOption((Option) it.next());
		it = createDatasetOptions().getOptions().iterator();
		while (it.hasNext()) optionGroup.addOption((Option) it.next());
		optionGroup.setRequired(true);
		return optionGroup;
	}
	...
	@SuppressWarnings("static-access")
	private static OptionGroup createGranuleOptions() {
		OptionGroup optionGroup = new OptionGroup();
		optionGroup.addOption( OptionBuilder.withArgName( "granuleId" )
                .hasArg()
                .withLongOpt("granuleId")
                .withDescription("Granule Id" )
                .create("g"));	
		optionGroup.addOption( OptionBuilder.withArgName( "id1,id2,id3,..." )
                .hasArgs()
                .withValueSeparator(',')
                .withDescription("List of granule id" )
                .create("gl") );
		optionGroup.addOption( OptionBuilder.withArgName( "beginId:endId" )
                .hasArgs(2)
                .withValueSeparator(':')
                .withDescription("Range of granule id" )
                .create("gr") );
		optionGroup.setRequired(true);
		return optionGroup;
	}
	...
	private static Option createIndexOption() {
		return OptionBuilder.withArgName( "beginIndex:endIndex" )
                ...
                .hasArgs(2)
                .withValueSeparator(':')
                .withDescription("Begin & end Index to use with datasetId;" )
                .create("i");
	}
	...
	private static void createOptions() {
		...
		optionGroup.addOption(OptionBuilder.hasArg(false)
                .withDescription("Verify the archived entity by granuleId or datasetId;" )
                .create("verify"));
		optionGroup.addOption(OptionBuilder.hasArg(false)
                .withDescription("Delete the archived entity by granuleId;" )
                .create("delete"));
		optionGroup.addOption(OptionBuilder.hasArg(false)
                .withDescription("Relocate the archived entity by granuleId;" )
                .create("relocate"));
		optionGroup.addOption(OptionBuilder.hasArg(false)
                .withDescription("Locate granule IDs based on name or start/stop times;" )
                .create("locate"));
		optionGroup.addOption(OptionBuilder.hasArg(false)
                .withDescription("Scan for and process rolling store data;" )
                .create("rolling_store"));
		optionGroup.addOption(OptionBuilder.hasArg(false)
                .withDescription("Reassociate granules from one dataset to another;" )
                .create("reassociate"));
	...
	private static Options createRelocateCmdOptions() {
		...
		optionGroup.addOption(OptionBuilder.withArgName("force")
				.hasArg(false)
				.withLongOpt("force")
                .withDescription("Force the relocation of granules without prompting user" )
                .create("f"));	
		optionGroup.setRequired(false);
		options.addOptionGroup(optionGroup);
	...
	private static Options createRollingCmdOptions() {
		...
		options.addOption(OptionBuilder.hasArg(true)
                ...
                .withDescription("A start date (MM/dd/yyyy) with which only granules after should be processed." )
                .create("start"));
		options.addOption(OptionBuilder.hasArg(true)
				.withLongOpt("stop-date")
                .withDescription("A stop date (MM/dd/yyyy) with which only granules before should be processed." )
                .create("stop"));
		return options;
	}
	@SuppressWarnings("static-access")
	private static Options createReassociateCmdOptions() {
		...
		options.addOption(OptionBuilder.hasArg(true)
				.withLongOpt("pattern")
                .withDescription("A pattern to search granule names on, using '#' as wildcard characters." )
                .create("pattern"));
		options.addOption(OptionBuilder.hasArg(true)
				.withLongOpt("from-dataset")
                .withDescription("The dataset to which the granules currently belong." )
                .create("fd"));
		options.addOption(OptionBuilder.hasArg(true)
				.withLongOpt("to-dataset")
                .withDescription("The dataset to which the granules will be moved." )
                .create("td"));
		options.addOption(OptionBuilder.hasArg(false)
				.withLongOpt("test-only")
                .withDescription("Test only mode will not move files or change metadata. Only prinst out the new paths for a granule." )
                .create("test"));
		return options;
	}
	...
	private static Options createLocateCmdOptions() {
		...
		options.addOption(OptionBuilder.hasArg(true)
				.withLongOpt("dataset")
                .withDescription("The dataset on which to locate granules." )
                .create("d"));
		return options;
	}
	...
	private static Options createVerifyCmdOptions() {
		...
		options.addOption(OptionBuilder.hasArg(false)
                ...
                .withDescription("Validate references against dataset location policy" )
                .create("lp"));
		options.addOption(OptionBuilder.hasArg(false)
				.withLongOpt("checksum")
                .withDescription("When verifying a granule, also verify its checksum." )
                .create("cs"));
		options.addOption(OptionBuilder.hasArg(false)
                .withDescription("print verify usage" )
                .create("help"));
