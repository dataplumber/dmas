	...
	private List<Record> nameLookup(List<Record> allRecords) {
		...
		for(Record r : allRecords)
		{
			...
			if(prodId){
				...
				names = InventoryFactory.getInstance().getQuery().findDatasetByProductId(r.product_id);
				//System.out.println("size of names: " + names.size());
				if(names.size()==0){
					System.out.println("No dataset found for product: "+ r.product_id);
					log.debug("No dataset found for product: "+ r.product_id);
					r.dName=null;
				}
					...
					for(String n : names){
						log.debug("Multiple datasets name: " + n);
					...
					log.debug("Single datasets name: " + names.get(0));
					r.dName= names.get(0);
					newRecords.add(r);
				}
			}
				...
				if(useLegacy){
					log.debug("looking up pid: " + r.product_id);
					try{
						name = InventoryFactory.getInstance().getQuery().fetchDataset(r.product_id, new String()).getShortName();
					...
					try{
						...
						name = InventoryFactory.getInstance().getQuery().fetchDatasetByPersistentId(r.product_pers_id).getShortName();
						log.debug("Found item with name: " + name);
					}catch(NullPointerException npr){
						r.dName=null;
					}
...
//		allRecords.addAll(newRecords);
		 ...
		 try{
			    ...
			    FileWriter fstream = new FileWriter(outputPath);
			        ...
			        for(Record r : allRecords)
					{
						...
						{
							out.write("\t\t<Entry>\n");
							out.write("\t\t\t<DatasetName>"+r.dName+"</DatasetName>\n");
							...
							if(useLegacy || prodId)
								log.error("No dataset name found for product with id=" + r.product_id);
							else
								log.error("No dataset name found for product with id=" + r.product_pers_id);
						}
					}
			        out.write("\t</Entries>\n</Document>\n");
	...
	public void loadOptions(){
		...
		options.addOption ("leg", "legacyId", false, "Uses the legacy, numeric dataset_id.");
	    ...
	    options.addOption ("pid", "Product Id", false, "use if the ID in the inputfiles is a product ID. Otherwise Dataset IDs will be used.");
	}
