package gov.nasa.podaac.distribute.subscriber;
...
import gov.nasa.podaac.common.api.util.Errno;
import gov.nasa.podaac.common.api.util.GetOpt;
import gov.nasa.podaac.common.api.util.SystemProcess;
import gov.nasa.podaac.distribute.subscriber.api.*;
import gov.nasa.podaac.distribute.subscriber.cache.SubscriberXml;
...
public class Subscriber {
	...
	private gov.nasa.podaac.distribute.subscriber.api.Subscriber dataSubscriber;
	private gov.nasa.podaac.distribute.subscriber.api.DataRetriever dataRetriever;
	...
	private HashSet<Dataset> listOfDatasets = new HashSet<Dataset>();
	private HashSet<Granule> removeList;
	...
	protected void start()
	{
		...
		_createDatasetsFromNames();
		_createDatasetDirectories();
		...
		while(true)
		{
			...
			for(Dataset ds : listOfDatasets)
			{
				...
				removeList = new HashSet<Granule>();
				
				if(dataSubscriber.list(ds, ds.getLastListingTime()))
				{
					//_logger.info("Setting Last Listing Time["++"] for dataset " +ds.getShortName());
					ds.setLastListingTime(lastRunTime);
				}
				else
				{
					_logger.info("No new granules for dataset \""+ds.getShortName()+"\" were found in the previous query.");
				}
												
				for(Granule g: ds.getGranuleFiles())
				{
					List<String> gotten = new ArrayList<String>();
					_logger.info("Fetching granule: " + g.getName());
					_logger.info("Output directory: " + dataDir +File.separator + ds.getShortName());  // Added output directory so we know where the file got fetched to.
					try{
						gotten = dataRetriever.get(g, dataDir +File.separator + ds.getShortName());
					...
					catch(IOException ioe)
					{
						...
						_logger.error("IO Exception Thrown, Granule["+g.getId()+"] "+g.getName()+" not retrieved.");
						_postSig("Error Retrieving Granule["+g.getId()+"] "+g.getName()+". " + ioe.getMessage());
						//_postSig(EventType.Error, m_executionErrorCategory,"Error Retrieving Granule["+g.getId()+"] "+g.getName()+".", ioe.getMessage(), ds.getShortName());
					}finally{
						//move this to after execOnGranule if we want to go back to caching.
						removeList.add(g);
					}
					...
					try{
						if(!gotten.isEmpty())
						{
							...
							{					
								...
								String dname = ds.getShortName();
								execOnEachGranule(name, status, dname, fileList);
							}
						...
						else
						{
							...
							_postSig("Error Retrieving Granule["+g.getId()+"] "+g.getName()+".");
							//_postSig(EventType.Error, m_executionErrorCategory,"Error Retrieving Granule["+g.getId()+"] "+g.getName()+".", "No Data to Report", ds.getShortName());
						}
					}
					catch(Exception e)
					{
						...
						_postSig("Error on external execution of script on Granule["+g.getId()+"] "+g.getName()+". " + e.getMessage());
						//_postSig(EventType.Error, m_executionErrorCategory,"Error on external execution of script on Granule["+g.getId()+"] "+g.getName()+".", e.getMessage(), ds.getShortName());
				...
				ds.getGranuleFiles().removeAll(removeList);
				if(!ds.getGranuleFiles().isEmpty())
				{
					_logger.info("Writing unfetched granules to cache.");
					SubscriberXml.toXml(ds.getGranuleFiles(), dataDir +File.separator + ds.getShortName() + File.separator + "cacheFile.xml");
				}
				else{
					//delete the cacheFile?
					File cf = new File(dataDir +File.separator + ds.getShortName() + File.separator + "cacheFile.xml");
					if(cf.exists())
					{
						if(cf.delete())
						{
				...
				_postSigMessages(ds.getShortName());
			}
	...
	private void _createDatasetDirectories() {
	
		for(Dataset d: listOfDatasets)
		{
			File dsDir = new File(dataDir + File.separator + d.getShortName());
			if(!dsDir.exists())
			{
				if(dsDir.mkdir())
					_logger.info("Created dataset directory '"+d.getShortName()+"'");
				else
				{
					System.out.println("Could not create dataset directory '"+d.getShortName()+"'; Check permissions and run the subscriber again.");
					System.exit(5);
				}	
			}
			else
			{
				...
				String cacheName = dataDir + File.separator + d.getShortName() + File.separator + "cacheFile.xml";
				File cacheFile = new File(cacheName);
				if(cacheFile.exists())
				{
					_logger.info("Adding previously failed granules to dataset list from cache.");
					d.getGranuleFiles().addAll(SubscriberXml.fromXml(cacheName));
				}
	...
	private void execOnEachGranule(String gName, String gStatus, String dsName, String fileList) throws Exception {
		/*
	...
	private void _createDatasetsFromNames() {
	...
	private DataRetriever _setUpDataRetriever() {
		...
		try {
			...
			return (gov.nasa.podaac.distribute.subscriber.api.DataRetriever) clazz.newInstance();
	...
	private gov.nasa.podaac.distribute.subscriber.api.Subscriber _setUpSubscriber() {
		...
		try {
			...
			return (gov.nasa.podaac.distribute.subscriber.api.Subscriber) clazz.newInstance();
