...
function printDatasetListOld($listXML, $valueLabel, $fieldLabel, $idLabel) {
	$results = array();
        error_log("printDatasetList");
        error_log(gettype($listXML));
	foreach($listXML->field as $field) {
...
function printDatasetList($listXML, $valueLabel, $fieldLabel, $idLabel) {
        $results = array();
        foreach($listXML->dataset as $field) {
		$results[] = array($fieldLabel =>(int) $field->id, $valueLabel => $field->{'short-name'}->__toString());
        }
        print "{ \"identifier\":\"$fieldLabel\",
                \"label\": \"$valueLabel\",
                \"items\": \n";
...
if($type == "dataset") {
	...
	if(isset($_GET['dataset_datasetId']) ) {
		...
		$valueLabel = "dataset_shortName";
		error_log( "$INVENTORY_URL/datasets"."?".$params."\n");
		//$response = do_post_request("$INVENTORY_URL/$INVENTORY_PROCESS", $params);
		$response = do_get_request("$INVENTORY_URL/datasets", $params);	
		//print $response."\n";
		//$listXML = simplexml_load_string(simplexml_load_string($response)->Content);
