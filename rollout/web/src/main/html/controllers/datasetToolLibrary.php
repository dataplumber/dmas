	...
	foreach($sorted as $field) {
		...
		if(preg_match("/datasetPolicy\_.*/",$fieldName)) {
			...
			elseif($fieldName == "datasetPolicy_compressType") {
				$options = array("BZIP2", "GZIP", "NONE", "ZIP", "UNIX");				
				createComboField($fieldName, $required, $maxLength, $label, $tooltip, $options, $value);
			}
...
function printDatasetCoverageTable($manifest) {
	...
	foreach($sorted as $field) {
		...
		if(preg_match("/^datasetCoverage\_.*/",$fieldName)) {
			if($fieldName == "datasetCoverage_granuleRange360") {
                                $options = array("Y", "N");
                                createComboField($fieldName, $required, $maxLength, $label, "Y if granule range is from 0 to 360.", $options, $value);
                        }else{
                        if($fieldType == "char")
                                createCharField($fieldName, $required, $maxLength, $label, $tooltip, $value);
                        elseif($fieldType == "int" || $fieldType == "float" || $fieldType == "double")
                        ...
                        elseif($fieldName == "datasetCoverage_stopTime")
                                createDateField($fieldName, $required, $maxLength, $label, $tooltip, $value, "validateStartTime(arguments[0]);");
                        elseif($fieldType == "datetime")
                                createDateField($fieldName, $required, $maxLength, $label, $tooltip, $value);
			}
		}
			...
			if(preg_match("/datasetResource_description.*/", $fieldName)) {
				...
				print "var label = new dojo.create(\"div\", {innerHTML:\"".getLabel($fieldName).":  \", style:\"float:none;\"});
					var resourceTypeData = {identifier:\"value\",
							label:\"value\",
							items:[{value:\"PORTAL_DA_DIRECT_ACCESS\"},
								{value:\"PORTAL_DA_TOOLS_AND_SERVICES\"},
								{value:\"PORTAL_DA_READ_SOFTWARE\"},
								{value:\"PORTAL_DA\"},
								{value:\"PORTAL_DOC_ANNOUNCEMENTS\"},
								{value:\"PORTAL_DOC_USERS_GUIDE\"},
								{value:\"PORTAL_DOC_PROJECT_MATERIALS\"},
								{value:\"PORTAL_DOC_JOURNAL_REFERENCES\"},
								{value:\"PORTAL_DOC_ADDITIONAL_SITES\"},
								{value:\"PORTAL_DOC_KNOWN_ISSUES\"},
								{value:\"PORTAL_DOC_FAQS\"},
								{value:\"PORTAL_DOC\"},
								{value:\"Thumbnail\"},
	...
	foreach($manifest->field as $field) {
		...
		if(preg_match("/(datasetCollection\_.*)/", $fieldName, $matches)) {
			...
			if($fieldName == "datasetCollection_CollectionId") {
				...
				print "var label = new dojo.create(\"div\", {innerHTML:\"".getLabel($fieldName).":  \", style:\"float:none; display:none;\"});
					var typeData = {identifier:\"value\",
							label:\"value\",
							items:[{value:\"Y\"},
								{value:\"N\"}]};
					var rangeStore = new dojo.data.ItemFileWriteStore({data:typeData});
					var field = new dijit.form.ComboBox({maxLength:\"$maxLength\", style:\"float:none; display:none;\", id:\"$fieldName\"+num, name:\"$fieldName\"+num, $disableText required:$required, store: rangeStore, searchAttr:\"value\", labelAttr:\"value\"});
					labels.push(label);
					inputs.push(field.domNode);\n";
			}
