...
public class  ReadJasonGDRDir extends Object{
     ...
     private static String GEO_STRING = "";     
     
     
     public ReadJasonGDRDir(String input_dir, String GEO_STRING){
    	 ReadJasonGDRDir.GEO_STRING = GEO_STRING;
	  ...
	  FilenameFilter filter_gdr_version = new FilenameFilter(){
            ...
            if( name.matches("gdr_netcdf_[a-z]_(nasa|cnes)+"+ReadJasonGDRDir.GEO_STRING+"$")){
            //if( name.matches("gdr_netcdf_[a-z]_(nasa|cnes)_geodetic")){
	  ...
	  //"gdr_netcdf_[a-z]_(nasa|cnes)$"
          //Pattern NASAorCNES_pattern   = Pattern.compile("gdr_(nasa|cnes)_");
          Pattern NASAorCNES_pattern   = Pattern.compile("gdr_netcdf_[a-z]_(nasa|cnes)");
	  ...
	  for (int i=0; i<dir_list.length; i++){
	       ...
	       try {
	            ...
	            log.info("looking for cycle directories in: " + i + " of:" + dir_list.length + " " + examine_gdr_version_dir.toString() );
	            ...
	            for (int j=0; j<cycle_dirs.length; j++){
	                 ...
	                 String[] pass_files = examine_cycle.list(remove_starting_dot);
		              ...
		              this.PassFilesList.add(fullPath + "/" + pass_files[0]);
                              ...
                              String auth = NASAorCNES_match.group(1);
                              System.out.println("FIND NASA OR CNES:"+dir_list[i] + " dauth: " + auth);
                         }
