...
public class releaseData{
  ...
  public static void main(String [] args) {
     ...
     try{
          ...
          geodetic = p.getProperty("geodetic", "0");
     ...
     if(geodetic.equals("1"))
	  	GEO_STRING="_geodetic";
     ...
     try {
          ...
          while( DBCycleIterator.hasNext() ){
               ...
               //if non gedecitc but processing geodetics, continune
               if(geodetic.equals("1") && Integer.valueOf(tempCycle.getCycle_num()) < 500)
            	   continue;
               //if geodetic but not processing them, continue
               if(geodetic.equals("0") && Integer.valueOf(tempCycle.getCycle_num()) >= 500)
            	   continue;
               /*
                * Commented out because we no longer receive the binary GDR...
                */
//               if(tempCycle.getCnes_approved().equals("Y") && tempCycle.getNasa_approved().equals("Y") &&
//                  tempCycle.getGDR_release_date().equals("(N/A)") 
//                 )
//               {
//            	   	log.debug("Moving geodetic");
...
//                    source = new File(gdrDir+"/gdr_"+tempCycle.getAuthor().toLowerCase()+"_"+tempCycle.getVersion().toLowerCase()+GEO_STRING+
//                                           "/c"+tempCycle.getCycle_num());
//                    
//                    destination = new File(gdrReleaseDir+"/gdr_"+tempCycle.getVersion().toLowerCase()+GEO_STRING+"/c"+
...
//               }
               ...
               {
            	   log.debug("Moving sgdr geodetic");
                    ...
                    source = new File(sgdrDir+"/sgdr_"+tempCycle.getVersion().toLowerCase()+
                                           "_"+tempCycle.getAuthor().toLowerCase()+GEO_STRING+
                                           "/c"+tempCycle.getCycle_num());
                    destination = new File(sgdrReleaseDir+"/sgdr_"+tempCycle.getVersion().toLowerCase()+GEO_STRING+"/c"+
                                         tempCycle.getCycle_num());
	       ...
	       //if sgdrnetCDF_staged move sgdr netCDF data
                 ...
                 )
               {
                    ...
                    source = new File(sgdrDir+"/sgdr_netcdf_"+tempCycle.getVersion().toLowerCase()+
                                           "_"+tempCycle.getAuthor().toLowerCase()+GEO_STRING+
                                           "/c"+tempCycle.getCycle_num());
                    destination = new File(sgdrReleaseDir+"/sgdr_netcdf_"+tempCycle.getVersion().toLowerCase()+GEO_STRING+"/c"+
                                         tempCycle.getCycle_num());
                 ...
                 )
               {
            	   log.debug("Moving NetCDF");
                    ...
                    source = new File(gdrDir+"/gdr_netcdf_"+tempCycle.getVersion().toLowerCase()+
                                           "_"+tempCycle.getAuthor().toLowerCase()+GEO_STRING+
                                           "/c"+tempCycle.getCycle_num());
                    destination = new File(gdrReleaseDir+"/gdr_netcdf_"+tempCycle.getVersion().toLowerCase()+GEO_STRING+"/c"+
                                         tempCycle.getCycle_num());
                 ...
                 )
               {
                    ...
                    source = new File(gdrDir+"/gdr_ssha_netcdf_"+tempCycle.getVersion().toLowerCase()+
                                           "_"+tempCycle.getAuthor().toLowerCase()+GEO_STRING+
                                           "/c"+tempCycle.getCycle_num());
                    destination = new File(gdrReleaseDir+"/gdr_ssha_netcdf_"+tempCycle.getVersion().toLowerCase()+GEO_STRING+"/c"+
                                         tempCycle.getCycle_num());
